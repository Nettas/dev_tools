...
RUN apt-get update && apt-get install -y software-properties-common add-apt-repository universe curl python-pip python-dev build-essential ruby wget jq bash-completion apt-transport-https sudo gnupg2 unzip libguestfs-tools ca-certificates qemu-utils git tmux openssh-server vim software-properties-common && \
